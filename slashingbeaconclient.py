...
class AttesterSlashingWithinSpan(BeaconClient):
    ...
    first_attestation: Dict[spec.ValidatorIndex, Tuple[spec.Checkpoint, spec.Checkpoint]]
    ...
    def produce_attestation_data(self, validator_index, validator_committee, epoch_boundary_block_root, head_block, head_state) -> Optional[spec.AttestationData]:
        ...
        if validator_index not in self.first_attestation:
            ...
            print(f"[BEACON CLIENT {self.counter}] Save first checkpoint seen for Validator {validator_index}")
            self.first_attestation[validator_index] = (attestation_data.source, attestation_data.target)
            return attestation_data
        ...
        if validator_index not in self.attestation_target_inside_span:
            ...
            if attestation_data.source.epoch > self.first_attestation[validator_index][0].epoch \
                    and attestation_data.source.epoch > self.first_attestation[validator_index][1].epoch:
                print(f"[BEACON CLIENT {self.counter}] Save designated inside-span attestation target checkpoint for Validator {validator_index}")
                self.attestation_target_inside_span[validator_index] = attestation_data.target
            return attestation_data
        ...
        if attestation_data.target.epoch > self.attestation_target_inside_span[validator_index].epoch:
            print(f"[BEACON CLIENT {self.counter}] Produce spanning attestation for Validator {validator_index}")
